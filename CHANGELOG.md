...
| **v3.9** | **Aug 24 2026** | **Bulk upload publish bug fix (status correctly set only on success). Admin QV image URL fix (ImageKit absolute URLs). Search now excludes drafts, handles array occasions. Delete flow shows ImageKit cleanup guide. Weaving Days removed everywhere. Type added to public product drawer. GI-certified → Silk Mark Certified sitewide (tags, badges, data, text). Card Gradient From/To removed from data model + all UI. Weave Type / Technique removed from product edit, admin QV, and public drawer. Offer Valid Until made internal — offer badge auto-hides after expiry date. Product drawer dynamically hides 4th image slot when only 3 photos present. Bulk upload template updated (offerValidUntil removed, silkMark added, giTagged retired). Export to Excel added to Product Manager (SheetJS, all fields, date-stamped filename). Blouse fields (blouseIncluded, blouseLength) added to bulk upload template — readable in both new-add and update modes; blousePiece auto-computed on commit.** |
